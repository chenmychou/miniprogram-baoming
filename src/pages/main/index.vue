...
export default {
  ...
  onLoad () {
    ...
    this.formData.majorData = this.$mp.query.id !== '0' ? this.majorListMap[this.$mp.query.id] : ''
  },
  methods: {
    // 刷新图形验证码
    changeIdCardKey () {
    ...
    blurIdNumber (e) {
      ...
      if (num.length === 18) {
        ...
        let autoGenderData = num.substr(16, 1)
      ...
      if (num.length === 15) {
        ...
        let autoGenderData = num.substr(13, 1)
        this.formData.genderData = autoGenderData % 2 === 0 ? this.genderList[1] : this.genderList[0]
        const nowYear = new Date().getFullYear()
    ...
    confirmForm () {
      ...
      if (validate(JSON.stringify(params), JSON.stringify(this.formDataMap))) {
        wx.navigateTo({
          url: '../info/main?data=' + JSON.stringify(postData)
        })
      }
    },
    cancelForm () {
